...
- Analysis row caps are configurable per session via `Engine.SetRowCaps(maxQuery, maxExport)` (0 → package default `config.DefaultMaxQueryRows` 10 000 / `config.DefaultMaxExportRows` 1 000 000). `QuerySQLToCSV` uses the export cap because its rows never enter the chat — keep the export and chat caps distinct (ADR-0029). Bindings wires both in `switchAnalysis` and re-applies live on `SaveSettings`. `MaxAnalyzeRows` (analyze-data's pure memory backstop) stays a package var and is intentionally not configurable.
