...
- **Container sandbox (opt-in)** — six `sandbox-*` tools that execute shell or Python in a per-session `podman`/`docker` container with `/work` mounted from the session's data dir, MITL-gated, network-off by default. See [sandbox-execution.md](docs/en/sandbox-execution.md) for the macOS setup guide.
