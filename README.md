...
- **Bundled scripts** — `file-info`, `preview-file`, `list-files`, `weather`, `get-location`, `write-note`. Auto-installed on first launch via `go:embed`; user customizations are preserved. Optional examples (not auto-installed; copy from `app/internal/bundled/tools/examples/` into your tool dir): `web-search` (gem-search), `generate-image` (gem-image), `search-kb-gem` ([gem-rag](https://github.com/nlink-jp/gem-rag) — Vertex AI Gemini RAG), `search-kb-lite` ([lite-rag](https://github.com/nlink-jp/lite-rag) — local LLM RAG). KB examples require the corresponding CLI installed and a pre-indexed corpus.
