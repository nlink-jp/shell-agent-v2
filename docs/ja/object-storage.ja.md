...
> ステータス: 実装済み — Phase 1〜4 は v0.1.10 / v0.1.11 でマージ済み。
> 以降、`sync.RWMutex` による同時書込み安全性 (v0.1.18) と、
> テキスト系 blob / CSV のインアプリプレビュー (v0.1.19) を追加。
...
### 7.4 per-session Objects（Data ディスクロージャ内、v0.1.11+）

情報表示リデザインに伴い、独立した Objects サイドバーパネルは廃止。
直接閲覧はチャットペイン上部の **Data** 折りたたみディスクロージャ
（現在選択中のセッションでフィルタ）内で行う。物理ストレージは
従来同様コンテンツアドレスのセントラル blob ストアで変わらず、
ユーザに見える表面のみ per-session 化した。背景は
[information-display-redesign.ja.md](information-display-redesign.ja.md)
参照。

**リスティング**: `bindings.GetSessionObjects(sessionID)` が
`objstore.ListBySession` をラップし、各オブジェクトのメタデータ
（`ID / Type / MimeType / OrigName / CreatedAt / SessionID / Size`）
を新しい順で返す。カードグリッドが各オブジェクトを 1 タイルとして表示:

- **image** — `ObjectImage` でサムネイル。クリックで lightbox
- **report** — クリックでドキュメントアイコン (📄) →
  `bindings.GetObjectText(id)` で markdown を取得、既存の全画面
  レポートビューアで表示
- **blob** — 型アイコン (📦)。**テキスト系 MIME**（`text/*`,
  `application/json`, `application/xml`, `application/javascript`,
  `application/x-ndjson`）の場合はクリックでインアプリプレビュー
  ダイアログ（§7.5 参照）。それ以外の blob は no-op のまま
  — Export は引き続き利用可能

**バルク / 単体削除**は両方とも、別ボタンの Yes / No 確認を伴う
インライン確認 UI を経由する（旧来の「同じボタンを 2 回クリック」
パターンは、削除位置の誤クリックがそのまま確認状態のボタンに
到達しうる問題から廃止）。

- バルク: カード選択 → セクションヘッダの「Delete…」で赤い確認バー
  「Delete N item(s)?」+ [Delete] [Cancel]
- 単体: カード ✕ ホバーで現れる × → カード全体に背景ぼかしの
  オーバーレイ展開、[Yes] [No]

確認ボタン押下で `bindings.DeleteObject(id)` /
`DeleteObjects(ids)` を呼び出す。

`ObjectReferences` は確認 UI 表示前に呼ばなくなった（明示的な
2 ボタン確認を安全網として十分とみなす）。bindings 自体は
「使用中ですよ」警告を出したい呼び出し側のために残置。
...
### 7.5 BlobPreview ダイアログ (v0.1.19+)

従来「blob = クリック no-op」だったが、CSV をはじめ用途のあるテキスト系
成果物が見えないままなのは、データ分析を売りにするツールとして筋が悪い。
v0.1.19 で `app/frontend/src/dialogs/BlobPreview.tsx` を追加:

- **CSV / TSV** は本物の HTML `<table>` で描画（先頭 200 行 × 30 列。
  超過分はフッタで明示）。パーサーは RFC 4180 形式の引用符
  （`""` エスケープ、引用内の区切り文字、引用内改行）を CSV ライブラリ
  なしで実装。フル RFC 対応や TSV 以外の区切り文字をサポートしたい
  場合は本物のパーサーに切り替える前提
- **その他テキスト MIME** は等幅 `<pre>` ビュー
- **ソースサイズ上限**: `GetObjectText` 経由でテキスト取得し、ダイアログに
  到達する前に 100 KB で切り詰め。ダイアログ自体もテーブルで 200 行、
  pre で約 200 KB の上限。フッタで切り詰めを通知
- **sticky テーブルヘッダ**: thead `<th>` セルは v0.1.19 で導入した
  per-theme 不透明色 `--bg-sticky-header` を使用。スクロール時に
  下のセルが透けない（旧 `--bg-hover` は全テーマで半透明）

`App.tsx` の `onPreviewObject` がディスパッチの単一窓口で、`obj.type` と
MIME によって routing する — image → Lightbox / report → ReportViewer /
text 系 blob → BlobPreview / それ以外は no-op。
...
### Phase 1: objstore コア更新 (完了, v0.1.10)
- [x] ObjectType 定数追加 (TypeImage, TypeBlob, TypeReport)
- [x] ObjectMeta に SessionID 追加、全 Save で設定
- [x] ListBySession(sessionID) メソッド
- [x] DeleteBySession(sessionID) メソッド
- [x] インデックスリビルドのタイプ保持

### Phase 2: レコード移行 (完了, v0.1.10)
- [x] Record の `ImageURLs []string` → `ObjectIDs []string`
- [x] AddUserMessage をオブジェクト ID 受け入れに変更
- [x] SendWithImages: objstore に先に保存、ID を渡す
- [x] BuildMessages: 最新画像を objstore から解決

### Phase 3: LLM ツール (完了, v0.1.10)
- [x] list-objects ツール実装
- [x] get-object ツール (画像リコールマーカー対応)
- [x] create-report を objstore に保存

### Phase 4: フロントエンド (完了, v0.1.10〜v0.1.11)
- [x] ReactMarkdown object: URL 解決コンポーネント
- [x] 画像キャッシュと遅延読み込み
- [x] レポート保存時の object: URL → インライン base64 解決

### Phase 5: 同時実行ハードニング (完了, v0.1.18)
- [x] `objstore.Store` のマップアクセスを `sync.RWMutex` で
      保護。post-response goroutines と次のツール呼び出しが
      `concurrent map writes` で panic していた問題を解消

### Phase 6: テキスト blob プレビュー (完了, v0.1.19)
- [x] テキスト系 MIME 用の `BlobPreview` ダイアログ
      （CSV/TSV はテーブル、その他テキストは pre）
- [x] `App.tsx::onPreviewObject` が type + MIME でルーティング
- [x] sticky `<th>` 用の不透明テーマ別変数
      `--bg-sticky-header`（テーブルプレビュー + DB 行プレビュー）
