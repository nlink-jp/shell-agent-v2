...
スイート」と認識させる。計 8 本 — 実行系 2 本 (`run-shell` /
`run-python`)、データ移送系 3 本 (`write-file` / `copy-object` /
`register-object`)、分析ブリッジ 2 本 (`load-into-analysis` /
`export-sql`)、内省 1 本 (`info`)。
...
サンドボックスの `/work` ディレクトリから DuckDB 分析エンジンへの
ブリッジ。`sandbox-run-python` で生成されたファイル（例: 上流データ
から計算した CSV）を、明示的な `load-data` ステップなしに
`query-sql` / `describe-data` / `suggest-analysis` でクエリ可能に
する。ホスト側マウント経由でファイルを読むので、コンテナホップは
不要。
...
```json
{
  ...
  "parameters": {
    "type": "object",
    "properties": {
      "path":       {"type": "string", "description": "/work 配下のデータファイルパス (例: 'sales.csv')"},
      "table_name": {"type": "string", "description": "分析データベースに作成するテーブル名。英数字とアンダースコアのみ。"}
    },
...
`load-into-analysis` の逆方向。DuckDB 分析エンジンに対する SELECT を
実行し、結果を `/work` に CSV として直接書き出す。動機は、クエリ結果を
チャットにテキストとして貼り付けて LLM 経由で `sandbox-run-python` に
渡すのが、無駄（モデルのツール予算を消費）かつ損失（数値の再
フォーマット、大規模結果の切り詰め）であること。本ツールはチャット
チャネルを完全に迂回し、`QuerySQLToCSV` 経由でホスト側マウントに
直接 CSV ストリームを書き込む。
...
```json
{
  ...
  "parameters": {
    "type": "object",
    "properties": {
      "sql":       {"type": "string", "description": "実行する SELECT クエリ"},
      "file_path": {"type": "string", "description": "/work 配下の出力先パス (例: 'tokyo_sales.csv')。親ディレクトリは必要なら自動作成。"}
    },
...
実装: `analysis.QuerySQLToCSV(query, writer)` が read-only SQL ガード
を通し、行を 1 回 walk して `csvFormat` で値を整形、UTF-8 CSV を書く。
ハンドラは他のファイル書き込みツールと同じ `safeWorkPath` の `/work/`
正規化を経由し、`wrote N rows (cols=...) to /work/<rel>` を返すので
LLM は続く `sandbox-run-python` で直ちにファイル参照できる。
