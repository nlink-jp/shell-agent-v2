...
> 作成日: 2026-04-24
> ステータス: 歴史的資料 — shell-agent v2 の実装着手前に範囲を
> 定める目的で起草。**意図** の正準リファレンスとして残置するが、
> アーキテクチャ・エージェントループ・メモリ・サンドボックス・
> オブジェクトリポジトリは v0.1.19 までに実装・改修されており、
> 現状は各機能の設計ドキュメントと CHANGELOG.md を参照のこと。
> 本 RFP は機能リリースに伴って書き換えない方針 — 「なぜ v2 が
> このような設計か」は本書、「現状で何ができるか」は per-feature
> 設計ドキュメントと changelog で確認する。
