...
> **ステータス:** ドキュメントリファクタ進行中 (2026-05-13)。
> 下記リンクは予定された `reference/` および `adr/` 配置を指して
> おり、後続のマイグレーションコミットで有効化されます。それ
> までは本ディレクトリ直下の既存フラットファイル
> (`architecture.ja.md`, `tool-registry-refactor.ja.md` 等) が
> authoritative です。
