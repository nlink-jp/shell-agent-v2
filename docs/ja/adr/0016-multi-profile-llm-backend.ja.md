...
Settings ダイアログに新タブ「LLM Profiles」を追加。既存の「General」
タブ内の「Local LLM」「Vertex AI」セクションは v0.12.0 で削除する
(プロファイルが正規の保管先になるため、2 つのサーフェスで編集できると
混乱する)。
...
**ライブ適用 (Save ボタンなし)** — macOS ネイティブな慣習に合わせ、
Settings ダイアログの他タブと同じ振る舞い:

- テキスト入力は **blur** (フォーカスアウト) で commit。新しい endpoint
  を打鍵 → Tab で別フィールドへ → config.json がアトミックに書かれる。
  Save / Cancel ボタンなし、dirty state 管理なし。
- ドロップダウン (Default side) は **change** で即時 commit。
- ヘッダーに各 commit 後 1.8 秒だけ「Saved ✓」インジケータが表示され、
  ユーザーは変更が通った確認ができる。
- エラーは 5 秒表示 (ユーザーが読む時間を確保)。
...
Edit フロー: blur / change のたびに `Bindings.UpdateProfile(id, req)`
が走り、config.json がアトミックに書かれる。メモリ内の他のセッションには
影響なし — 次のセッションロード時に変化が見える (プロファイル解決は
LoadSession 時)。*現在アクティブな* プロファイルを編集している場合は
`UpdateProfile` が `agent.RestartLLMBackend()` を呼ぶので、endpoint /
project ID / retry policy の変更がアプリ再起動なしで即時反映される。
...
| Event | Payload | When |
|-------|---------|------|
| `agent:profile:changed` | `{profile_id, profile_name, default_backend}` | `/profile <name>` の後、`SwitchSessionProfile` の後、**任意の LoadSession 後** (常に発火 — フロントエンドの status pill が今ロードされたセッションのプロファイルを反映するため、activeProfileID が同一でも発火する。これがないと同じプロファイルを共有する 2 セッション間の切替で pill が前セッションの表示のまま固まる)。削除済プロファイル → デフォルト fallback も同じ経路で発火。 |
...
ステータスバーには 1 つの統合 pill — 現セッションのプロファイル名と
バックエンドを `/` で繋いだもの。現在のバックエンド色 (Local = 緑、
Vertex = 青) に着色:

```
Local アクティブ:        [Default / Local]
Vertex アクティブ:       [Production GCP / Vertex]
```

実装メモ: 設計初稿は profile と backend を 2 つの別バッジに分けて
いたが、E2E フィードバックで統合に変更 — 両方のバッジが同じ popover
を開くため「1 アクションに 2 つのアフォーダンス」は冗長と判明。統合
pill は両情報を可視に保ちつつ、クリックターゲットも 1 つに集約。

pill をクリックするとステータスバー下にアンカーされた
...
設計根拠: ステータス pill は「自分は何で動いているか」のパッシブ情報、
かつそのまま「それを変える」アクティブサーフェスのクリックターゲット
を兼ねる。1 つのアフォーダンス、1 つのクリック。
