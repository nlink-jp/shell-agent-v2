...
- `objectMarkdown.tsx` が `react-markdown` から
  ...
  `internal/frontendlint/check_test.go` の
  `TestDefaultUrlTransformIsCentralised` で実装: フロントエンド
  ソースツリーを走査し、他のファイルが import していたら fail。
  4 つ目の箇所がオーバーライドを複製したらマージ前にテストが catch。

### 6.4 手動スモーク (リリース前)

`make build` 完了後、生成された macOS バイナリに対して実行。
リポジトリの `{reference,adr,history}/` 三層 doc ルール により
`testing/` ディレクトリは作れないので、チェックリストは ADR 内に置く。

セットアップ — クリーンなデータディレクトリ:
```
mv ~/Library/Application\ Support/shell-agent-v2 ~/Library/Application\ Support/shell-agent-v2.bak
```
(終了時に戻したければバックアップから restore。)

アプリ起動後:

1. **画像ハッピーパス。** 画像をチャットにドラッグ → 参照するレポート
   作成を依頼 → `![alt](object:ID)` がインライン画像で描画、クリック
   で lightbox が開くことを確認。
2. **`![alt]` → markdown チップ。** `.md` をチャットにドラッグ →
   「同じ markdown image 構文で添付した document を指して」と依頼
   → 壊れた画像アイコンの代わりにクリック可能 `📝` チップが表示され、
   クリックで `ReportViewer` が開くことを確認。
3. **`[title]` → markdown チップ。** 同じ `.md` で、今度は
   「返答内で `[name](object:ID)` を使って document をリンクして」
   と依頼 → assistant バブル内インラインでチップが描画され、クリック
   で `ReportViewer` が開くことを確認。
4. **`[title]` → インライン画像。** ステップ 1 と同じ画像で、今度は
   `[my-image](object:imgID)` を書くようモデルに依頼 → レンダラが
   暗黙の意図を honour してインライン画像で描画 (チップではない)
   ことを確認。
5. **Blob save-as。** 非テキストファイルに `register-object` 適用
   → モデルに `[zip](object:ID)` で参照させる → クリックで OS の
   保存ダイアログが現れることを確認。
6. **オブジェクト不在。** レポートがまだ参照しているオブジェクトを
   データパネルで削除 → セッション再オープン → チップが muted
   (`object-link-missing` modifier の「missing object」) で描画
   され、クリックが no-op であることを確認。
7. **`ReportViewer` 内ネストチップ。** モデルに別レポートにリンクする
   レポートを作らせる → インラインレポートカードから 1 つ目を開く
   → 内側のチップをクリック → `ReportViewer` のコンテンツが置換
   されることを確認 (意図通り — §4.3)。
8. **Streaming 描画中。** `[name](object:ID)` を含む返答をモデルが
   まだストリーミング中の段階で、閉じ括弧がストリームに来た瞬間に
   チップが現れることを確認 (App.tsx:streaming も同じ components
   マップを使うようになった)。
9. **Cmd popup 描画。** cmd-popup パネルに markdown を出すチャット
   コマンド (`/whoami` 等) を発火 → その出力に含まれる `object:`
   参照が正しく描画されることを確認 (App.tsx:cmd-popup も同じ
   components マップを使うようになった)。
10. **エクスポート self-containment。** 画像参照と markdown 参照を
    混在するレポートで **Save** クリック → 生成 `.md` を外部
    エディタ (VS Code、Marked) で開く → 画像が
    `data:image/...;base64,…` でインライン化、markdown 参照が
    `(object:ID)` のまま残る (`data:text/markdown` blob にならない)
    ことを確認。
11. **セッション切替時のキャッシュクリア。** セッション A 開いた
    状態で meta キャッシュ (DevTools → memory) にエントリがあること
    を確認 → セッション B に切替 → キャッシュが空になっている
    (`App.tsx:handleLoadSession` 内の `clearObjectMetaCache` 配線が
    動いた) ことを確認。

いずれかが失敗したらリリースタグを打たない。
