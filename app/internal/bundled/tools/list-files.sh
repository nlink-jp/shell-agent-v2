#!/bin/bash
# @tool: list_files
# @description: List the contents of one directory using `ls -la`. Returns the raw long-format ls output as plain text (mode bits, owner, group, size in bytes, mtime, name) — one entry per line, hidden entries (names starting with `.`) included. NOT recursive: only the immediate children of `path`. For deeper inspection use file_info on a specific file, or preview_file to read text contents. Defaults to /tmp if `path` is omitted. The session work directory (where write_note / register_object write files) is at $SHELL_AGENT_WORK_DIR; pass that as `path` to list it.
