...
- **ツールを使いたい** → [README.ja.md](README.ja.md) に
  インストール・機能・基本使用方法を記載。
- **内部構造を理解したい** →
  [docs/ja/architecture.ja.md](docs/ja/architecture.ja.md) で
...
ディスパッチフロー — は
[docs/ja/architecture.ja.md](docs/ja/architecture.ja.md) から
...
トレードオフ — は `docs/en/` (+ `docs/ja/` ミラー) で設計ノートを
書く。日常的な挙動変化は該当する既存リファレンスを更新する。
...
[docs/ja/tool-registry-refactor.ja.md](docs/ja/tool-registry-refactor.ja.md)
に記載。
