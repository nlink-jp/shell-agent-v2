...
- **`analyze-data` 行数上限修正 (v0.4.4)** — スライドウィンドウ要約が対話的 10,000 行チャット出力 cap を継承していたため、それより大きいテーブルでは起動すらせず、機能が意味を持つ唯一の領域で死んでいた。新しい `Engine.QuerySQLForAnalyze` 経路 (`MaxAnalyzeRows = 1_000_000` バックスタップ付き) で `query-sql` / `query-preview` / `quick-summary` のチャット出力 cap は維持しつつ、`analyze-data` だけはスライドウィンドウが実際に意味を持つサイズのテーブルを歩けるようにした。新エラーメッセージ (はるかに高い cap を踏んだ場合のみ到達) は `LIMIT` ではなく事前集約を提案する — `LIMIT` は分析を先頭 N 行に静かに切り詰めて機能を潰すため。[analyze-data-row-cap.ja.md](docs/ja/analyze-data-row-cap.ja.md) 参照
...
- **`save-query` による絞り込み解析 (v0.8.0)** — `analyze-data` は従来テーブル全体に対する sliding-window 解析しかできなかった。新ツール `save-query` は `SELECT` 結果を新規派生ベーステーブルとしてマテリアライズし、その派生テーブル名を `analyze-data` に渡すことで、絞り込み後のサブセット (直近 24 時間・エラー行のみ・特定顧客のイベント等) を deep に解析できる。派生テーブルは `list-tables` でロード済みテーブルと並んで現れ、`analysis.duckdb` 内に保存されるため export/import でも移動し、`reset-analysis` で他のテーブルと一括 wipe される。エンジンスキーマ変更なし、バンドルフォーマット version bump なし。[saved-query-tables.ja.md](docs/ja/adr/0013-saved-query-tables.ja.md) 参照
