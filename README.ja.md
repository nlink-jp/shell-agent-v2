...
- **内蔵スクリプト** — `file-info`, `preview-file`, `list-files`, `weather`, `get-location`, `write-note`。初回起動時に `go:embed` から自動インストール、ユーザーカスタマイズは保護。任意の参考例 (自動インストールされない、`app/internal/bundled/tools/examples/` からツールディレクトリにコピーして使用): `web-search` (gem-search)、`generate-image` (gem-image)、`search-kb-gem` ([gem-rag](https://github.com/nlink-jp/gem-rag) — Vertex AI Gemini RAG)、`search-kb-lite` ([lite-rag](https://github.com/nlink-jp/lite-rag) — ローカル LLM RAG)。KB 例は対応する CLI のインストールとインデックス済みコーパスが前提
